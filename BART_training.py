import polars
import transformers
from transformers import BartForConditionalGeneration, BartTokenizerFast, Seq2SeqTrainer, Seq2SeqTrainingArguments
import torch
import gc
import accelerate
...
def answer_question_generator(batch):
    input_encodings  = tokenizer(batch["claim_reviewed"],padding='max_length',truncation=True,return_tensors="pt",max_length=MAX_TOKENS)
    target_encodings = tokenizer(text_target = batch["question"], padding='max_length',truncation=True,return_tensors="pt",max_length=MAX_TOKENS) 
